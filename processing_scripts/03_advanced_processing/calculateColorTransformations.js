...
const colorSpaces = ["rgb", "p3", "rec2020"]
...
const FILE_COLOR_AVGS_O = "../../model/test.json"; // Path for the output
...
// Make color objects for all colors
for(const langData of grouped_lang){
  for(const term of langData.terms){
    for(const response of term.values){
      ...
      } else if(response.colorSpace == "p3"){
        // For now we naively treat all color spaces as srgb until we have enough data to estimate transformation
        response.responseColor = new Color({
          space: "p3", coords: [response.r, response.g, response.b]
        })
      } else if(response.colorSpace == "rec2020"){
        // For now we naively treat all color spaces as srgb until we have enough data to estimate transformation
        response.responseColor = new Color({
          space: "rec2020", coords: [response.r, response.g, response.b]
        })
      } else {
        throw new Error("can't find color space: " + response.colorSpace)
      }
...
for(const colorSpace of colorSpaces){
  for(const background of backgrounds){
    for(const version of versions){
      ...
      console.log(colorSpace, background, version)
      
      // filter data
      const filtered_grouped_lang = []
      for(const langData of grouped_lang){
        const langDataCopy = {...langData}
        langDataCopy.terms = []
        for(const term of langData.terms){
          const termCopy = {...term}
          termCopy.values = termCopy.values.filter(response => 
                response.colorSpace == colorSpace && 
                response.background == background && 
                response.studyVersion.startsWith(version))
          if(termCopy.values.length > 0){
            langDataCopy.terms.push(termCopy)
          }
        }
        if(langDataCopy.terms.length > 0){
            filtered_grouped_lang.push(langDataCopy)
        }
      }

      
      
      console.log("filtered_grouped_lang", filtered_grouped_lang)

      // find avg color for each term
      for(const langData of filtered_grouped_lang){
        if(!(langData.key in colorAvgs)){
            colorAvgs[langData.key] = {}
        }
        for(const term of langData.terms){
          if(!(term.key in colorAvgs[langData.key])){
            colorAvgs[langData.key][term.key] = {}
          }

          const avgL = term.values.map(response => response.responseOklch.l).reduce((a, b) => a + b) / term.values.length
          const avgC = term.values.map(response => response.responseOklch.c).reduce((a, b) => a + b) / term.values.length
          const avgH = term.values.map(response => response.responseOklch.h).reduce((a, b) => a + b) / term.values.length
          
          const suffix = colorSpace + "_" + background + "_v" + version + "_"

          colorAvgs[langData.key][term.key][suffix + "avgL"] = avgL
          colorAvgs[langData.key][term.key][suffix + "avgC"] = avgC
          colorAvgs[langData.key][term.key][suffix + "avgH"] = avgH
          colorAvgs[langData.key][term.key][suffix + "responseCount"] = term.values.length
  
        }
      }
      
      console.log("colorAvgs", colorAvgs)

    //   for(const labBinSize of LAB_BIN_SIZES){


    //     console.log("calculating full colors for bin size " + labBinSize, colorSpace, background, version)
...
const flattenedAvgsData = []

for(const [lang, langData] of Object.entries(colorAvgs)){
    for(const [term, termData] of Object.entries(langData)){
        flattenedAvgsData.push({
            ...{lang: lang, term: term},
            ...colorAvgs[lang][term]
        })
    }
...
fs.writeFileSync(FILE_COLOR_AVGS_O, JSON.stringify(flattenedAvgsData))