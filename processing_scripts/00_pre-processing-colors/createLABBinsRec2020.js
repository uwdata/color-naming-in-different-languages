...
function findClosestRGBToLAB(lab, rgbs){
    ...
    for(let i = 0; i < rgbs.length; i++){
        const rgb = rgbs[i]
        let rgb_lab = 
            (new Color({space: "rec2020", coords: [rgb.r, rgb.g, rgb.b]}))
            .to("oklab")
...
for(let labBinSize of LAB_BIN_SIZES){
        ...
        for(let l_bin = 0; l_bin < BIN_L_N; l_bin++){
            ...
            for(let a_bin = -(BIN_AB_N-1)/2; a_bin <= (BIN_AB_N-1)/2; a_bin ++){
                ...
                for(let b_bin = -(BIN_AB_N-1)/2; b_bin <= (BIN_AB_N-1)/2; b_bin ++){
                    ...
                    let centerRGB = (new Color({space: "oklab", coords: [l_bin_center, a_bin_center, b_bin_center]}))
                        .to("rec2020")
                    let binInfo = {
                        l_bin: l_bin,
                        a_bin: a_bin,
            ...
            for(let c_bin = 0; c_bin <= BIN_C_N; c_bin ++){
                ...
                for(let h_bin = 0; h_bin < hue_bin_num; h_bin ++){
                    ...
                    let binInfo = {
                        ...
                        h_max: h_bin_max,
                        center_rgb: {
                            r: lhcCenterRGBVals.r,
                            g: lhcCenterRGBVals.g,
                            b: lhcCenterRGBVals.b
                        },
                        rgbs: []
                    }
                    if(!labBinInfo[l_bin]){
    ...
    for(let r = 0; r <=255; r+= 2){//r++){
        r % 20 == 0 ? console.log("r", r): ""
        for(let g = 0; g <= 255; g+= 2){//g++){
            for(let b = 0; b <= 255; b+= 2){//b++){
                let bin
                if(labBinSize.type == "cube" || labBinSize.type == "box"){
                    let lab = (new Color({space: "rec2020", coords: [r/255, g/255, b/255]})).to("oklab")
                    //let lab = oklab.rgbToOklab({r: r, g: g, b: b});
                    // update max and min lab values
                    updateMaxMins({l: lab.l, a: lab.a, b: lab.b})
                    ...
                    let lch = (new Color({space: "rec2020", coords: [r/255, g/255, b/255]})).to("oklch")
                    //let lab = oklab.rgbToOklab({r: r, g: g, b: b});
                    // update max and min lab values
                ...
                bin.rgbs.push((new Color({space: "rec2020", coords: [r/255, g/255, b/255]})));
            ...
            for(let a_bin = -(BIN_AB_N-1)/2; a_bin <= (BIN_AB_N-1)/2; a_bin ++){
                for(let b_bin = -(BIN_AB_N-1)/2; b_bin <= (BIN_AB_N-1)/2; b_bin ++){
                    ...
                    if(bin_info.rgbs.length == 0){
                        ...
                        if(center_rgb.r > 1 || center_rgb.r < 0 ||
                            center_rgb.g > 1 || center_rgb.g < 0 ||
                            center_rgb.b > 1 || center_rgb.b < 0
                        ){
                            ...
                            bin_info.representative_lab = //oklab.rgbToOklab({r: closest_rgb.r, g: closest_rgb.g,b: closest_rgb.b})
                                (new Color({space: "srgb", coords: [closest_rgb.r, closest_rgb.g, closest_rgb.b]}))
                                .to("oklab")
                        }
                ...
                for(let h_bin = 0; h_bin < hue_bin_num; h_bin ++){
                    ...
                    if(bin_info.rgbs.length == 0){
                        ...
                        if(center_rgb.r > 1 || center_rgb.r < 0 ||
                            ...
                            representative_lab = (new Color({space: "srgb", coords: [closest_rgb.r, closest_rgb.g, closest_rgb.b]}))
                                .to("oklab")
    ...
    fs.writeFileSync(FILE_O_LAB_BINS+"_rec2020_"+(labBinSize)+".json", JSON.stringify(labBinInfo, null, 2));
}