...
## Blur / No-Blur
For each file of binned data, there is also a blurred version, where a small gaussian blur was run on bins. We arbitrarily aim for a sum of blur weights to be about 2.5 (1.0 contribution from the center bin we are currently looking at, and 1.5 contribution from nearby bins).
...
- **num_bins_X:** The number of bins (size X) that we kept for this language. (we require a minimum amount of data in a bin to keep info about it. So with the blur having weights totals of ~2.5, we can include more bins)
...
- **langAbv:** Two letter abbreviation of the language
- **binL/binA/binB OR binL/binC/binH:** Index of Color Bin
...
The "full_color_map_saliency_bins_X.json" files contains summary color naming information about each bin (bin size X) for each language (does not include all terms, so these files are smaller). 

{"lang":"English (English)","langAbv":"en","binL":1,"binC":0,"binH":0,"lab":"0.1,,","saliency":0,"maxpTC":1,"majorTerm":"black","commonTerm":"black","avgTermColor":"rgb(34,31,34)","topTerms":[{"term":"black","commonTerm":"black","pTC":1}]},

- **lang:** Language (long version)
- **langAbv:** Two letter abbreviation of the language
- **binL/binA/binB OR binL/binC/binH:** Index of Color Bin
- **lab:** A string of the representative Oklab value for this bin (center if it maps to valid rgb, otherwise closest valid rgb in bin to center of bin)
- **saliency:** The saliency (negative entropy base-2) of color naming in this bin
- **maxpTC:** The maximum probability of a term (t) given this color bin (c) (P(t|c)), that is the P(t|c) for the most common color name for this bin.
- **majorTerm:** The most common simplified matching term used for this bin (not display name)
- **commonTerm:** The most common standardized name for the majorTerm (display name)
- **avgTermColor:** The average sRGB color value for the majorTerm
