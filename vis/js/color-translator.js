...
function updateTerms(){
	...
	for(const langName in colorNamesAbrv){	
		...
		if(langNameAbv == urlParams.get("start_lang")){
			$("#startLang").val(langName)
		}
		if(langNameAbv == urlParams.get("dest_lang")){
			$("#destLang").val(langName)
		}
	}
...
function updateTranslation(){
	...
	$("#startColorInfo").html(generateColorInfoString(term, allColorInfo[currentStartLangAbrv][term],currentStartLangAbrv));
	...
	for(const i in matchedTranslations){
		...
		var urlParams = new URLSearchParams("?" + window.location.hash.replace("#", ""));
		urlParams.set("term", matchedName);
		urlParams.set("start_lang", currentEndLangAbrv);
		urlParams.set("dest_lang", currentStartLangAbrv);
		const termLink = urlParams.toString()
		//window.location.hash = urlParams.toString().replace("?", "");

		$("#results").append(generateColorInfoString(matchedName,  allColorInfo[currentEndLangAbrv][matchedName], currentEndLangAbrv, termLink));
		// convert distance to 0.02 (expected Just noticeable distance in oklab)
		const JNDs = matchedTranslations[i].dist / 0.02

		const roundedJNDs = JNDs.toPrecision(3)

		$("#results").append(`<small>Estimated<span 
					title='Noticeable Difference is calculated as the name-color distribution distance in Oklab color space, where "Just Noticeable Difference" is expected to be 0.02'>*</span>
					Noticeable Difference: 
			 ${roundedJNDs}</small><br><br>`);
	}
...
function generateColorInfoString(name, colorInfo, langAbv, termLink){
	let str = "";
	...
	str += "<strong style='padding-left:5px;'>"
	str += (termLink ? `<a class="subtle-link" href="#${termLink}">` : "") + colorInfo.commonColorName + `${(termLink ? `</a>` : "")}</strong>`;
	...
	if('colorNodes16' in colorInfo){
		str += generateColorGrid(colorInfo.colorNodes16)
	}else if('colorNodes9' in colorInfo){
	...
	str += `This name represents ${(colorInfo.totalColorFraction*100).toPrecision(2)}% of color names in this language.<span 
					title='This name was entered ${colorInfo.numRecords.toLocaleString()} times when we showed colors from the whole color spectrum'>*</span><br>`;
	...
	} else if(langAbv == "en" && name == "fuchsia" && colorInfo.commonColorName != "fuchsia"){
		str += '<small>Note: English dictionaries say "fuchsia" is the correct spelling, but we show what the majority of our participants typed in, which is a different spelling.</small><br>'
	}
...
function generateColorGrid(nodes){
	...
	for(let i = 0; i < nodes.length; i++){
		for(let j = 0; j < nodes.length; j++){
			let node = nodes[i][j];
			str += "<div style='";
			str += "width: "+90/nodes.length+"px; height: "+90/nodes.length+"px;float:left;"
